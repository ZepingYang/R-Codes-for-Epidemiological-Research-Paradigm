...
dt <- data.frame(  # <Remove this line when apply to your research>
  Var1_continuous = rnorm(100, mean = 50, sd = 10),  # 正态分布模拟连续变量
  Var2_categorized = sample(c(1, 2), size = 100, replace = TRUE),  # 随机生成分类变量 (1或2)
  Var3_categorized = sample(c(1, 2, 3), size = 100, replace = TRUE)  # 随机生成分类变量 (1, 2或3)
)
